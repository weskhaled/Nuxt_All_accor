...
// watch(() => hotelDetails.value, (valueHotelDetails) => {
//   loadingGetRooms.value = true
//   errorGetRooms.value = null
//   rooms.value = []

//   if (!valueHotelDetails || !valueHotelDetails.offers?.length || !modelValue.value)
//     return

//   const hotelId = valueHotelDetails.hotel?.id
//   const dateIn = valueHotelDetails.offers[0].period?.dateIn
//   const nights = valueHotelDetails.offers[0].period?.nights
//   const adults = valueHotelDetails.offers[0].effectiveOccupancy?.adults
//   const childrenAges = valueHotelDetails.offers[0].effectiveOccupancy?.childrenAge
//   useFetch(`/api/hotels/${hotelId}/rooms?adults=${adults}&dateIn=${dateIn}&nights=${nights}&childrenAges=${childrenAges}`).then(({ data, error }) => {
//     if (data.value)
        ...
        <div class="mx-auto max-w-3xl px-3 py-3 lg:max-w-7xl">
          ...
          <div class="m-0 mb-2 w-full flex-none lg:sticky lg:top-17.25 lg:float-right lg:mb-0 lg:max-w-sm lg:w-xs xl:w-sm">
            <div class="w-full overflow-hidden border border-gray-200 rounded-sm bg-white/75 shadow-sm dark:border-gray-700 dark:bg-gray-8/75">
              ...
              <div class="px-5 pb-5">
                <h1 class="m-0 mt-2 text-xl font-bold tracking-tight sm:text-2xl">
                  {{ hotelDetails?.hotel?.name }}
                </h1>
                ...
                <p class="mb-2 hidden font-normal text-gray-700 dark:text-gray-400">
            ...
            <div class="w-full flex flex-1">
              <div v-if="rooms && !loadingGetRooms && !errorGetRooms" class="w-full flex flex-col space-y-2">
                <div v-for="room in rooms" :key="room.code" :class="[selectedRoomOffer?.roomCode === room.code ? 'border-blue-3 dark:border-blue-7 dark:bg-gray-8 bg-white' : 'border-gray-2 dark:border-gray-7 dark:bg-gray-8/65 bg-white/65']" class="border rounded-sm p-1 shadow-sm">
                  ...
                  <div>
                    <h3 class="md:text-md my-4 flex items-center gap-4 font-semibold after:h-px before:h-px after:flex-1 before:flex-1 after:bg-zinc-3/20 before:bg-zinc-3/20 after:content-[''] before:content-['']">
                      CHOISIR LE TARIF DE VOTRE CHAMBRE
                    </h3>
                    <div class="pb-1">
                      <a-radio-group v-model="selectedTarif" class="w-full flex flex-col space-y-2">
                        <template v-for="item in room.referenceOffers" :key="item.code">
                              ...
                              >
                                ...
                                <div class="ml-2 w-full">
                                  <div class="custom-radio-card-title">
                                    {{ room.offers.find(o => o.code === item.code)?.label }}
                                  </div>
                                  <a-typography-text type="secondary">
                                    {{ room.offers.find(o => o.code === item.code)?.description }}
                                  </a-typography-text>
                                  <div text-right>
                                    <span class="text-lg font-light text-gray-900 lg:text-xl dark:text-white">
                                      {{ (room.offers.find(o => o.code === item.code)?.pricing?.amount?.afterTax).toLocaleString('fr-FR', { style: 'currency', currency: room.offers.find(o => o.code === item.code)?.pricing?.currency || 'EUR' }) }}
                                    </span>
                                  </div>
                                </div>
                              </div>
