...
<template>
  <Swiper
    ...
    :mousewheel="false" @swiper="onSwiper" @active-index-change="onSlideChange"
  >
    <SwiperSlide v-for="(slide, index) in sliders" :key="index" class="relative h-full flex">
      <header class="h-full w-full flex items-center justify-center text-black dark:text-light">
        ...
        />
        <div
          class="absolute right-0 top-0 z-9 h-full w-full opacity-20"
        ...
        <div
          class="relative relative top-0 z-10 mx-auto w-full px-8 transition-opacity transition-top-50 duration-0s container"
    ...
    <template v-if="sliders.length > 1">
      ...
      <nav class="nav-slit">
        <a
          class="prev" href="javascript:;"
          @click="activeSlideIndex < 1 ? sliderRef.slideTo(sliders.length) : sliderRef.slidePrev()"
        >
        ...
        <a
          class="next" href="javascript:;"
          @click="activeSlideIndex + 1 >= sliders.length ? sliderRef.slideTo(0) : sliderRef.slideNext()"
        >
          <span class="icon-wrap">
